...
fn in_range(point: &Point, rect: &Rectangle) -> bool {
    ...
    point.longitude >= cmp::min(lo.longitude, hi.longitude)
        && point.longitude <= cmp::max(lo.longitude, hi.longitude)
        && point.latitude >= cmp::min(lo.latitude, hi.latitude)
        && point.latitude <= cmp::max(lo.latitude, hi.latitude)
}
